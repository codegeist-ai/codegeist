...
| `ollama` | Dedicated `spring-ai-starter-model-ollama`. | Install/start Ollama or Testcontainers-backed Ollama, pull the selected local model, and confirm local resource availability. | None for local daemon; optional base URL. | `local-free`; costs are local compute, disk, and download time. | Not a hosted remote provider. | `local`. | 2026-06-01 | [SA1], [O1]. First Codegeist real-provider candidate. |
