#!/usr/bin/env bash
# codegeist-install-macos.sh - install the macOS x64 Codegeist release archive.
#
# Why this exists:
# - Gives users a small curl-downloadable bootstrap script for GitHub Release
#   assets without adding Homebrew, notarization, signing, or OS-native installers.
# - Installs the complete native archive directory so sidecar libraries remain
#   beside the executable while the user runs a stable `codegeist` command.
#
# Inputs:
...
#
# Side effects:
# - Downloads codegeist-macos-x64.tar.gz and SHA256SUMS.txt into a temporary
...
readonly archive_name='codegeist-macos-x64.tar.gz'
readonly package_name='codegeist-macos-x64'
readonly checksum_name='SHA256SUMS.txt'
readonly install_root="${CODEGEIST_INSTALL_DIR:-$HOME/Library/Application Support/Codegeist}"
...
verify_archive() {
  ...
  fail 'shasum or sha256sum is required on PATH'
}
...
DYLD_LIBRARY_PATH="\$CODEGEIST_HOME\${DYLD_LIBRARY_PATH:+:\$DYLD_LIBRARY_PATH}"
